...
class _OACut(object):
    def __init__(
        self,
        nonlin_expr,
        expr_vars: Sequence[VarData],
        coefficients: Sequence[ParamData],
        offset: ParamData,
    ):
    ...
    def update(
        ...
        too_large: float,
        safety_tol: float,
    ) -> Tuple[bool, Optional[VarData], Optional[float], Optional[str]]:
        """
        Parameters
        ----------
        var_vals: Sequence[float]
            The values of the variables at the point where a cut should be 
            generated
        relaxation_side: RelaxationSide
        too_small: float
            If a coefficient in the cut is smaller than this value (but strictly greater than 0) 
            and the bounds on the corresponding variable are set, the coefficient will be 
            changed to 0 and the constant will be modified based on the variable bounds.
        too_large: float
            If a coefficient in the cut is larger than this value, 
            then a cut will not be generated

        
        Returns
        -------
        success: bool
            if False, a cut was not successfully added
        v: Optional[VarData]
            if success if False, and the issue was a large coefficient,
            this indicates which variable had a large coefficient
        c: Optional[Union[complex, float]]
            if success is False, and the issue was a large constant,
            this indicates the value of the constant
        msg: Optional[str]
            if success is False, this message provides some information
            about why a cut could not be generated
        """
...
class BaseRelaxationData(BlockData):
    ...
    def add_oa_point(
        self,
        var_values: Optional[
            Union[Tuple[float, ...], Mapping[VarData, float]]
        ] = None,
    ):
        """
        ...
        Parameters
        ----------
        var_values: Optional[Union[Tuple[float, ...], Mapping[VarData, float]]]
        """
        if self._has_a_convex_side():
    ...
    def add_cut(
        ...
        Returns
        -------
        new_con: pyomo.core.base.constraint.ConstraintData
        """
        rhs_vars = self.get_rhs_vars()
