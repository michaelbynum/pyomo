...
class ProblemWriter_cpxlp_temp(AbstractProblemWriter):
...
class _objectOrderer(object):
    ...
    def sorted_ids(self):
        return sorted(iterkeys(self.objOrder), key=self.objOrder.__getitem__)
...
class ProblemWriter_cpxlp(object):
    ...
    def __init__(self):
        ...
        self.eq_template = "= %"+self._precision_string+'\n\n'
        self.leq_template = "<= %"+self._precision_string+'\n\n'
        self.geq_template = ">= %"+self._precision_string+'\n\n'
        self.sos_template = "%s:%"+self._precision_string+"\n"
        self.var_bound_template = "  %s <= %s <= %s\n"
    ...
    def _print_model_LP(self, config, model, output, labeler):
        eq_template = self.eq_template
        leq_template = self.leq_template
        geq_template = self.geq_template
        sos_template = self.sos_template
        var_bound_template = self.var_bound_template

        x_labeler = LabelWrapper(labeler, 'x%s')
        ce_labeler = LabelWrapper(labeler, 'c_e_%s_')
        cl_labeler = LabelWrapper(labeler, 'c_l_%s_')
        cu_labeler = LabelWrapper(labeler, 'c_u_%s_')
        rl_labeler = LabelWrapper(labeler, 'r_l_%s_')
        ...
        output.write( ("min" if obj.is_minimizing() else "max") +
        ...
        for con in sorted(model.component_data_objects(Constraint, active=True),
            ...
            if con.equality:
                ...
                output.write(sym(con, ce_labeler) + ":\n")
                self._print_standard_repn(
                    output, repn, config, False, varOrder, sym, x_labeler)
                ...
                if lb is None:
                    output.write(sym(con, cu_labeler) + ":\n")
                    self._print_standard_repn(
                        output, repn, config, False, varOrder, sym, x_labeler)
                    output.write(leq_template % (ub - repn.constant))
                elif ub is None:
                    output.write(sym(con, cl_labeler) + ":\n")
                    self._print_standard_repn(
                        output, repn, config, False, varOrder, sym, x_labeler)
                    ...
                    body = StringIO()
                    self._print_standard_repn(
                        body, repn, config, False, varOrder, sym, x_labeler)
                    output.write(sym(con, rl_labeler) + ":\n"
                                 + body.getvalue()
                                 + geq_template % (lb - repn.constant))
                    output.write(sym(con, ru_labeler) + ":\n"
                                 + body.getvalue()
                                 + leq_template % (ub - repn.constant))
        ...
        #
        # Process (but do not write) SOS constraints
        #
        sosBuffer = []
        allowableSOS = set()
        if config.allow_sos1:
            allowableSOS.add(1)
        if config.allow_sos2:
            allowableSOS.add(2)
        for sos in sorted(
                model.component_data_objects(SOSConstraint, active=True),
                key=conOrder.object_sort_key):
            if sos.level not in allowableSOS:
                raise ValueError(
                    "Solver does not support SOS level %s constraints" % (level))
            if hasattr(soscondata, 'get_items'):
                sos_items = list(soscondata.get_items())
            ...
            sosBuffer.append('%s: %s::\n' % (sym(sos, s_labeler), sos.level))
            for vardata, weight in sos_items:
                weight = _get_bound(weight)
                if weight < 0:
                    raise ValueError(
                        "Cannot use negative weight %f "
                        "for variable %s is special ordered "
                        "set %s " % (weight, vardata.name, sos.name))
                if vardata.fixed:
                    raise RuntimeError(
                        "SOSConstraint '%s' includes a fixed variable '%s'. "
                        "This is currently not supported. Deactive this "
                        "constraint in order to proceed."
                        % (sos.name, vardata.name))
                sosBuffer.append(
                    sos_template % (sym(vardata, x_labeler), weight))
        ...
        integer_vars = []
        binary_vars = []
        for var_id in varOrder.sorted_ids():
            varSymbol = symbol_map.byObject.get(var_id, None)
            if varSymbol is None:
                # A variable passed to the column_order that was not
                # found in the constriants
                continue
            var = symbol_map.bySymbol.get(varSymbol)

            if var.fixed:
                if not output_fixed_variable_bounds:
                    raise ValueError(
                        "Encountered a fixed variable (%s) inside an active "
                        "objective or constraint expression on model %s, which "
                        "is usually indicative of a preprocessing error. Use "
                        "the IO-option 'output_fixed_variable_bounds=True' to "
                        "suppress this error and fix the variable by "
                        "overwriting its bounds in the LP file."
                        % (var.name, model.name))
            ...
            if var.is_binary():
                binary_vars.append(varSymbol)
            elif var.is_integer():
                integer_vars.append(varSymbol)
            ...
            lb = _get_bound(var.lb)
            ub = _get_bound(var.ub)
            output.write(var_bound_template % (
                _no_negative_zero(lb) if var.has_lb() else '-inf',
                varSymbol,
    ...
    def _print_standard_repn(self, output, repn, config, is_objective,
                             varOrder, sym, labeler):
